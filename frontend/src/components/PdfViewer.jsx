...
// Configurar el worker de PDF.js desde un CDN seguro
...
export default function PdfViewer({ url, height = '500px', className = '' }) {
    ...
    return (
        <div className={`w-full flex flex-col items-center rounded-xl overflow-hidden border border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-900 ${className}`}>
                ...
                {loading && !error && (
                    <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-100/50 dark:bg-slate-900/50 z-10 backdrop-blur-sm">
                        <div className="flex flex-col items-center gap-3 text-slate-500 dark:text-slate-400">
                            ...
                            <span className="text-sm font-medium animate-pulse">Cargando PDF...</span>
                        </div>
                    </div>
                )}
            ...
            {numPages && numPages > 1 && (
                <div className="w-full flex items-center justify-between px-4 py-2 border-t border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shrink-0">
                    <button
                        onClick={() => setPageNumber(p => Math.max(1, p - 1))}
                        disabled={pageNumber <= 1}
                        className="w-10 h-10 flex items-center justify-center rounded-lg bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 disabled:opacity-30 hover:bg-slate-200 dark:hover:bg-slate-600 transition-all active:scale-90"
                    >
